...
The R session materials are stored in the following read-only directory: "~/team/bootcamp/2018/R session materials". Copy the "data" and "exercises" subfolders (and others if you want) to a directory in your home folder.

<small>You can do this either using Bash or the GUI in the "Files" tab in the bottom-right pane or RStudio.</small>
...
> - **logical**: `TRUE` or `FALSE` <br><small>(When co> -coerced into numeric type, `TRUE`=`1` and `FALSE`=`0`)</small>
...
You can go to the Files tab in the bottom right window in RStudio and find the directory you want. Then under the More menu, there is an option to set the current directory as the working directory. Or you can use the `setwd` command as follows:
...
- Set your working directory: this should be the folder in which your copies of the "data" and "exercises" folders are stored.
...
>     - Index the data frame: `gapminder5[gapminder5$continent=="Asia",]`
>     - Use the `subset()` function: `subset(gapminder5, subset=continent=="Asia")`
...
> - When we do more intensive data cleaning and wrangling tasks in R, we will often want to save the resulting data frame as a data file.
> - You can use `write.csv()` from base R and `write_csv()` from `readr` to write data files.
