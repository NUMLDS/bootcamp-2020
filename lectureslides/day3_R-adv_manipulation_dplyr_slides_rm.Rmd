...
- select by name: 
  - `select(df, variable_1, variable_3)`
- select by position: 
  - `select(df, c(1, 3, 10))`
- select by range:
  - `select(df, variable_1:variable_10)` or `select(df, 1:10)`
- drop variables with `-`
  - `select(df, -variable_7)`
...
```{r multi mutate}
tmp <- mutate(long_merged_energy, log_output = log(output), output2 = output^2, output3 = output^3)
head(tmp)
```
...
long_merged_energy %>% 
  ...
  summarize(mean_log_output = mean(log_output, na.rm = T))
```
...
merged_energy %>% 
  select(-datetime) %>% 
  mutate(total_output = rowSums(., na.rm = T)) %>% 
  summarize(total_output = sum(total_output, na.rm = T))
```
...
merged_energy %>% 
  select(-datetime) %>% 
  mutate(total_output = rowSums(merged_energy, na.rm = T)) %>% 
  summarize(total_output = sum(total_output, na.rm = T))
```
...
- Using the (wide) merged CA energy data `merged_energy` do the following with pipes:
  ...
  3. Find the mean output for `total_hydro`
...
- Use your knowledge of `dplyr` to find the mean output for small hydro, large hydro, biogas, and biomass
